...
const Home = ({ route, navigation}) => {
        ...
        return (
            <View style={{ flexDirection: 'row', marginVertical: SIZES.padding * 2 }}>
                <View style={{ flex: 1 }}>
                    ...
                    <Text style={{ ...FONTS.body2, color: COLORS.white }}>{user_details.name}</Text>
                </View>
    ...
    function _onPress(item) {
        if (item.id === 1) {
            navigation.navigate("Items", {user_id: user_details.id});
        } else if (item.id === 2) {
            setUserId(JSON.stringify(user_details.id))
            console.log(userId);
        } else if (item.id === 4) {
